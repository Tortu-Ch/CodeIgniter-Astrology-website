...
class AppLogin extends CI_Controller
{
    ...
    {
        $bornPosition = '';
        $userData = $this->db_selectUser_query($id);
        if ($userData['bornPosition'] == 'i') {
            $output =  $this->sweph($userData['birthDate'], "2");
            preg_match_all('!\d+\.*\d*!', $output[0], $temp);
            $bornPosition = $temp[0][0];
        } else {
            $bornPosition = $userData['bornPosition'];
        }
        $lunarReturn = $this->lunar_return($userData['birthDate'], $bornPosition);
        $this->set_data($id, $bornPosition, $lunarReturn);
        $countDate = $this->count_date($lunarReturn);
        // $this->push_notification($countDate);
    }



    public function count_date($lunarReturn)
    {
        $currentDateTime = date('Y-m-d H:i:s');
        $currentDateTime = new DateTime($currentDateTime);
        $lunarReturn = new DateTime($lunarReturn);
        $temp = date_diff($currentDateTime, $lunarReturn);
        $countDate = $temp->format('%ddays and %h:%i:%s');
        var_dump($countDate);
        return $countDate;
    }

    public function push_notification($lunarReturn)
    {
        ...
        $token = '...';
        $title = 'Lunar Return';
        $body = 'you will get your lunar return in '.$lunarReturn.'.';
    ...
    public function db_selectUser_query($id)
    {
        $data = $this->get_data();
        $birthDate = '';
        $bornPosition = '';
        foreach ($data as $user) {
            if ($user['account']['id'] == $id) {
                $birthDate = $user['account']['birthday'];
                $timezone = $user['account']['timezone'];
                $bornPosition = $user['data']['born_position'];
                break;
            }
        }
        $offset = intval($timezone) * (60 * 60);
        $birthTimestamp = strtotime($birthDate);
        $utcTimestamp = date('Y-m-d H:i:s', $birthTimestamp - $offset);
        $userData = array('birthDate' => $utcTimestamp, 'bornPosition' => $bornPosition);
        return  $userData;
    }
    ...
    public function set_data($id, $bornPosition, $lunarReturn)
    {   $id = $id + 1;
        $update = [
            'Users/user'.$id.'/data/born_position' => $bornPosition,
            'Users/user'.$id.'/data/lunar_return' => $lunarReturn
        ];
        $firebase = $this->firebase->init();
        $db = $firebase->getDatabase();
        $db->getReference()->update($update);
        return ;
    }

    public function lunar_return($birthDate, $birthPosition)
    {
        // var_dump($birthPosition);
        $birthStamp = strtotime($birthDate);
    ...
    function cal_lunar($output, $birthPosition)
    {
        ...
        foreach ($output as $outPut) {
            $val = explode(",", $outPut);
            $absVal = abs(floatval($birthPosition) - $val[2]);
        ...
        if ($arrStep != '') {
            ...
            $tpTime = 60 * (floatval($birthPosition) - $tempPosition) / $iSpeed;
    ...
    function sweph($dateTime, $flag)
    {
        ...
        if ($flag == "2") {
            ...
            exec("swetest -edir$libPath -b$date -ut$time -p1 -eswe -fl -roundmin -g, -head ", $output);
        }
